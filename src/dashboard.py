...
@st.cache_data
def load_data():
    # Look for the file in the data folder, which is one level up (..) from src
...
if 'Prompt_ID' in df.columns:
    ...
    prompt_text = df[df['Prompt_ID'] == prompt_id].iloc[0]['Prompt_Text']
    st.info(f"**Prompt Text:** {prompt_text}")
    ...
    for i, model in enumerate(models):
        ...
        with cols[i]:
            clean_name = model.split('/')[-1] # Show just the model name
